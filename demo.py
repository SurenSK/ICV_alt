...
query_inputs_safe =  tokenizer("""This is a conversation between two people. Contex: I hate people. Response: """)
...
model = updated_wrapper.get_model(torch.stack(icvs_to_shift_safety,dim=1).cuda(), alpha = [args.alpha])
print('Style vectors have been added!\n')
generation_output = model.generate(
                        input_ids=torch.tensor(query_inputs_safe['input_ids']).unsqueeze(0).cuda(),
                        attention_mask=torch.tensor(query_inputs_safe['attention_mask']).unsqueeze(0).cuda(),
